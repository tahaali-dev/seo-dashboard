...
export default async function PageReport({
  ...
  scores.overall = Math.round(
    Object.values(scores.categories).reduce((a, b) => a + b, 0) / 6,
  );
  ...
  const renderIssueBadge = (severity: string) => {
    switch (severity) {
      ...
      case "LOW":
        return (
          <span className="bg-slate-500/20 text-slate-400 px-2 py-0.5 rounded text-[10px] font-bold">
            LOW
          </span>
        );
      default:
    ...
    return (
      <div className="flex flex-col sm:flex-row sm:items-start py-4 border-b border-slate-700/50 gap-4">
        ...
        <div className="w-full sm:w-2/3">
          <div className="text-white wrap-break-word mb-2">
            {value || <span className="text-slate-500 italic">None</span>}
          </div>
          {relevantIssues.map((i) => (
            <div
              key={i.id}
              className="flex items-center gap-2 mt-2 bg-slate-900/50 p-2 rounded-lg border border-rose-500/20"
            >
        ...
        <div className="flex items-center gap-4 border-b border-slate-800 pb-6">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <p className="text-slate-400">
              Site: {page.siteType} | Status: {page.crawlStatus}
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-slate-800/30 rounded-xl border border-slate-700 p-6">
            <h3 className="text-lg font-medium text-white mb-4 flex items-center gap-2">
              Page Structure{" "}
              <span className="text-sm font-normal text-slate-400">
                ({scores.categories["Page structure"]}%)
              </span>
            </h3>
            <div className="flex flex-col">
              {renderDataRow(
          ...
          <div className="bg-slate-800/30 rounded-xl border border-slate-700 p-6">
            ...
            {keywords.length > 0 && (
              <div className="mb-6">
                <span className="text-sm font-semibold text-slate-400 uppercase tracking-wider block mb-3">
                  Top Keyword Density (TF-IDF)
                </span>
                <div className="flex flex-wrap gap-2">
                  {keywords.map((kw: any, idx: number) => (
                    <div
                      key={idx}
                      className="bg-slate-900 border border-slate-700 px-3 py-1.5 rounded-lg flex items-center gap-2 shadow-sm group hover:border-indigo-500/50 transition-colors"
                    >
                      ...
                      <span
                        className={`text-xs font-bold ${parseFloat(kw.density) > 5 ? "text-rose-400" : "text-emerald-400"}`}
                      >
                        {kw.density}%
                      </span>
                    </div>
            ...
            <div className="flex flex-col border-t border-slate-700/50 pt-2">
              ...
              {renderDataRow(
                "HTML Size",
                metadata.htmlSize
                  ? `${Math.round(metadata.htmlSize / 1024)} KB`
                  : "N/A",
                "Page quality",
              )}
            </div>
          ...
          <div className="bg-slate-800/30 rounded-xl border border-slate-700 p-6">
            ...
            {brokenLinks.length > 0 && (
              <div className="mb-6 bg-slate-900/50 rounded-lg p-4 border border-slate-700/50">
                <span className="text-sm font-semibold text-slate-400 uppercase tracking-wider block mb-3">
                  Broken & Redirecting Outbound Links
                </span>
                    ...
                    return (
                      <li key={idx} className="flex items-start gap-2 text-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          <span
                            className={`font-mono text-xs mr-2 px-1.5 py-0.5 rounded ${isDead ? "bg-rose-500/20 text-rose-400" : "bg-amber-500/20 text-amber-400"}`}
                          >
