...
export default async function DashboardPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const getScoreForCategory = (cat: string, siteType: string, N: number) => {
    const issuesInCat = project.issues.filter((i: any) => i.category === cat && i.page?.siteType === siteType)
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        <header className="flex justify-between items-end border-b border-slate-800 pb-6 relative">
          <div className="absolute -top-4 left-0 flex items-center gap-4">
            <a href="/" className="text-slate-400 hover:text-white text-sm font-medium flex items-center gap-1 transition-colors">
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" /></svg>
              All Projects
            </a>
            <span className="text-slate-700">|</span>
            <a href="/new" className="text-indigo-400 hover:text-indigo-300 text-sm font-medium flex items-center gap-1 transition-colors">
        ...
        <CrawlerClient project={project} initialPages={project.pages} issues={project.issues} oldScores={oldScores} newScores={newScores} />
      </div>
    </div>
  )
}
