...
export default function CrawlerClient({ project, initialPages, issues = [], oldScores, newScores }: { project: any, initialPages: any[], issues?: any[], oldScores?: any, newScores?: any }) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
        <div className="lg:col-span-1 flex flex-col gap-4">
          {project.auditType === 'MIGRATION' && (
            <div className="bg-indigo-900/20 p-5 rounded-xl border border-indigo-500/20 flex-1 flex flex-col justify-center">
              <h3 className="text-indigo-400/80 font-medium mb-1 text-xs uppercase tracking-wider">Old Site Pages</h3>
          ...
          <div className="bg-emerald-900/20 p-5 rounded-xl border border-emerald-500/20 flex-1 flex flex-col justify-center">
            <h3 className="text-emerald-400/80 font-medium mb-1 text-xs uppercase tracking-wider">{project.auditType === 'FRESH' ? 'Crawled Pages' : 'New Site Pages'}</h3>
          ...
          <div className="bg-rose-900/20 p-5 rounded-xl border border-rose-500/20 flex-1 flex flex-col justify-center">
            <h3 className="text-rose-400/80 font-medium mb-1 text-xs uppercase tracking-wider">Critical Issues</h3>
        ...
        <div className="lg:col-span-3">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-semibold text-white">Global Analytics</h2>
            {project.auditType === 'MIGRATION' && (
              <div className="flex items-center space-x-2 bg-slate-900 p-1 rounded-full border border-slate-700">
                <button onClick={() => setAnalyticsSite('OLD')} className={`px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-colors ${analyticsSite === 'OLD' ? 'bg-indigo-500 text-white' : 'text-slate-400 hover:text-white'}`}>OLD SITE</button>
                <button onClick={() => setAnalyticsSite('NEW')} className={`px-4 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-colors ${analyticsSite === 'NEW' ? 'bg-emerald-500 text-white' : 'text-slate-400 hover:text-white'}`}>NEW SITE</button>
              </div>
            )}
          </div>
          <SEOScoreWidget scores={analyticsSite === 'OLD' ? oldScores : newScores} />
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-800/30 p-4 rounded-xl border border-slate-700">
        <div className="flex space-x-1 bg-slate-900 p-1 rounded-lg w-max border border-slate-700">
          <div className="flex items-center space-x-4">
            <button onClick={() => setActiveTab('ISSUES')} className={`px-6 py-2.5 rounded-lg font-medium text-sm transition-colors ${activeTab === 'ISSUES' ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800'}`}>Detected Issues ({issues?.length || 0})</button>
            <button onClick={() => setActiveTab('PAGES')} className={`px-6 py-2.5 rounded-lg font-medium text-sm transition-colors ${activeTab === 'PAGES' ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800'}`}>Crawled Pages ({pages.length})</button>
            {project.auditType === 'MIGRATION' && (
              <button onClick={() => setActiveTab('DIFF')} className={`px-6 py-2.5 rounded-lg font-medium text-sm transition-colors ${activeTab === 'DIFF' ? 'bg-indigo-500/20 text-indigo-400' : 'text-slate-400 hover:bg-slate-800'}`}>Migration Diff</button>
            )}
          </div>
        </div>
          ...
          return (
            <div className="space-y-4">
              {Object.entries(grouped).sort((a, b) => {
                ...
                return (
                  <div key={cat} className="bg-slate-800/30 rounded-xl border border-slate-700 overflow-hidden transition-all duration-200 shadow-sm">
                    <button 
                    ...
                    >
                      <div className="flex items-center gap-3">
                        <span className="text-lg font-semibold text-white">{cat === 'Static' ? 'Static Pages' : `${cat} Pages`}</span>
                        <span className="bg-slate-700 text-slate-300 text-xs font-bold px-2 py-0.5 rounded-full">{catPages.length}</span>
                      </div>
