...
export default async function DiffReport({
  params,
}: {
  params: Promise<{ id: string; oldId: string; newId: string }>;
}) {
  ...
  const renderMetricDiff = (
    ...
    return (
      <div className="grid grid-cols-3 gap-4 py-3 border-b border-slate-700/50 items-center">
        ...
        <div className="text-slate-300 wrap-break-word pr-4 border-r border-slate-700/50">
          {oldVal || "N/A"}
        </div>
        <div
          className={`wrap-break-word flex items-center gap-2 ${isPositive ? "text-emerald-400" : isNegative ? "text-rose-400" : "text-slate-300"}`}
        >
          {newVal || "N/A"}
          {isPositive && (
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex items-center gap-4 border-b border-slate-800 pb-6">
          ...
          <div className="flex-1 overflow-hidden">
            ...
            <p className="text-slate-400">
              Comparing identical URL paths between Old and New site.
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 p-6 rounded-xl border border-slate-700 flex justify-between items-center">
          <div className="text-center flex-1 border-r border-slate-700">
            ...
            <div className="text-xs text-indigo-400 mt-2 truncate px-4">
              {oldPage.url}
            </div>
          </div>
          ...
          <div className="shrink-0 px-8 text-center">
            <div
              className={`text-2xl font-bold ${scoreDiff > 0 ? "text-emerald-400" : scoreDiff < 0 ? "text-rose-400" : "text-slate-400"}`}
            >
              {scoreDiff > 0
                ? `+${scoreDiff}`
                : scoreDiff < 0
          ...
          <div className="text-center flex-1 border-l border-slate-700">
            ...
            <div className="text-xs text-emerald-400 mt-2 truncate px-4">
              {newPage.url}
            </div>
          </div>
        </div>
        ...
        <div className="bg-slate-800/30 rounded-xl border border-slate-700 overflow-hidden">
          ...
          <div className="p-4 space-y-2">
            <h3 className="text-lg font-medium text-white mb-4 mt-2">
              Content & Quality
            </h3>
            {renderMetricDiff(
              "Word Count",
            ...
            {renderMetricDiff(
              "Title Length",
              oldMeta.title?.length,
              newMeta.title?.length,
              "match",
            )}
            {renderMetricDiff(
              "Description",
